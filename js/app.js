...
async function apiRequest(action,payload=null,query={}){
  const url=getApiUrl(); if(!url) throw new Error('Primero pega la URL de Apps Script en Configuración.');
  const isPost=payload!==null;
  const options=isPost?{method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({action,...payload})}:{};
  const qs=new URLSearchParams({action,t:String(Date.now()),...query});
  const res=await fetch(isPost?url:`${url}?${qs.toString()}`,options);
  const data=await res.json(); if(!data.ok) throw new Error(data.error||'Error de conexión'); return data;
}
...
function drawerHtml(){
  ...
  <div class="form-section"><h3>Video corto</h3><div class="form-grid"><div class="form-group full"><label>Enlace del video</label><input name="video" type="url" placeholder="YouTube, Drive o MP4 directo"><span class="help-text">Recomendado: 5–30 segundos, 720p y menos de 10 MB. No se reproduce automáticamente.</span></div><div class="form-group full"><label>Descripción</label><textarea name="descripcion"></textarea></div></div></div></section>
...
navigate('productos');
if(getApiUrl()){ setTimeout(async()=>{try{await loadCatalogs();if(currentView==='productos')loadProductsPage();}catch(err){console.warn(err);}},250); }
