...
function bindConfiguration(){
  ...
  document.getElementById('saveApiUrl').onclick=()=>{setApiUrl(document.getElementById('appsScriptUrl').value);document.getElementById('connectionStatus').textContent=getApiUrl()?'URL configurada':'Modo demostración local';alert('Enlace guardado.');loadDriveConfiguration();};
  document.getElementById('testApiUrl').onclick=async()=>{setApiUrl(document.getElementById('appsScriptUrl').value);try{const d=await apiRequest('ping');alert(`Conexión correcta: ${d.message||'Google Sheets listo'}`);loadDriveConfiguration();}catch(err){alert(err.message);}};
