...
async function syncFromSheets(){ const btn=document.getElementById('syncSheetsBtn'); if(btn){btn.disabled=true;btn.textContent='Sincronizando…';} try{const data=await apiRequest('bootstrap'); db={products:Array.isArray(data.products)?data.products:[],colors:Array.isArray(data.colors)&&data.colors.length?data.colors:structuredClone(DEFAULT_DATA.colors)}; saveDb(); renderProductsShell(); alert(`Sincronización completada: ${db.products.length} producto(s) cargado(s) desde Google Sheets.`);}catch(err){alert('No se pudo sincronizar: '+err.message);}finally{const current=document.getElementById('syncSheetsBtn');if(current){current.disabled=false;current.textContent='↻ Sincronizar';}} }
