...
async function apiRequest(action,payload=null){ const url=getApiUrl(); if(!url) throw new Error('Primero pega la URL de Apps Script en Configuración.'); const options=payload?{method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({action,...payload})}:{}; const res=await fetch(payload?url:`${url}?action=${encodeURIComponent(action)}&t=${Date.now()}`,options); const data=await res.json(); if(!data.ok) throw new Error(data.error||'Error de conexión'); return data; }
async function syncFromSheets(){ const btn=document.getElementById('syncSheetsBtn'); if(btn){btn.disabled=true;btn.textContent='Sincronizando…';} try{const data=await apiRequest('bootstrap'); db={products:Array.isArray(data.products)?data.products:[],colors:Array.isArray(data.colors)&&data.colors.length?data.colors:structuredClone(DEFAULT_DATA.colors),categories:Array.isArray(data.categories)?data.categories:[],brands:Array.isArray(data.brands)?data.brands:[]}; saveDb(); renderProductsShell(); alert(`Sincronización completada: ${db.products.length} producto(s) cargado(s) desde Google Sheets.`);}catch(err){alert('No se pudo sincronizar: '+err.message);}finally{const current=document.getElementById('syncSheetsBtn');if(current){current.disabled=false;current.textContent='↻ Sincronizar';}} }
...
function renderConfiguration(){
  content.innerHTML=`<section class="module-page settings-module">
    <div class="page-actions">
      <div><p class="eyebrow">Configuración</p><h2 class="module-heading">Colores predeterminados</h2><p class="module-description">Crea una paleta reutilizable. Estos colores aparecen directamente al registrar variantes de productos.</p></div>
      <button class="btn btn-primary" id="saveColorBtn">+ Agregar color</button>
    </div>
    <section class="data-card connection-card">
      <div class="card-head"><h3>Conexión con Google Sheets</h3><span id="connectionStatus">${getApiUrl()?'URL configurada':'Modo demostración local'}</span></div>
      ...
      <p class="help-text">Al colocar la URL del Apps Script, Productos, Variantes, Mayoreo y Colores se cargarán desde las hojas reales.</p>
    </section>
    ...
    <section class="data-card linked-example">
      <div class="card-head"><h3>Ejemplo real enlazado</h3><span>Así se verá dentro de una variante de producto</span></div>
      <div class="variant-example"><span class="color-swatch large" id="exampleSwatch" style="--swatch:${escapeHtml(db.colors[0]?.hex||'#2563eb')}"></span><div><strong id="exampleName">${escapeHtml(db.colors[0]?.nombre||'Azul')}</strong><small>Bolsa de regalo · Talla grande · Stock 8</small></div><span class="status-pill active">● Activo</span></div>
    </section>
  </section>`;
  bindConfiguration();
}
function bindConfiguration(){
  ...
  document.getElementById('saveApiUrl').onclick=()=>{setApiUrl(document.getElementById('appsScriptUrl').value);document.getElementById('connectionStatus').textContent=getApiUrl()?'URL configurada':'Modo demostración local';alert('Enlace guardado.');};
  document.getElementById('testApiUrl').onclick=async()=>{setApiUrl(document.getElementById('appsScriptUrl').value);try{const d=await apiRequest('ping');alert(`Conexión correcta: ${d.message||'Google Sheets listo'}`);}catch(err){alert(err.message);}};
  const sync=(source)=>{
    let value=(source==='picker'?picker.value:text.value).trim();
    if(/^#[0-9a-fA-F]{6}$/.test(value)){
      picker.value=value; text.value=value.toUpperCase(); swatch.style.background=value;
      document.getElementById('exampleSwatch').style.setProperty('--swatch',value);
    }
    previewName.textContent=name.value.trim()||'Vista previa';
    document.getElementById('exampleName').textContent=name.value.trim()||'Color seleccionado';
  };
  picker.addEventListener('input',()=>sync('picker'));
  text.addEventListener('input',()=>sync('text'));
  name.addEventListener('input',()=>sync('name'));
  sync('picker');
  document.getElementById('saveColorBtn').onclick=async()=>{
    const nombre=name.value.trim(),hex=text.value.trim().toUpperCase();
    if(!nombre)return alert('Escribe el nombre del color.');
    if(!/^#[0-9A-F]{6}$/.test(hex))return alert('El color debe tener formato hexadecimal, por ejemplo #2563EB.');
    if(db.colors.some(c=>normalize(c.nombre)===normalize(nombre)))return alert('Ya existe un color con ese nombre.');
    const color={id:cryptoId(),nombre,hex}; db.colors.push(color); saveDb(); try{await pushColorToSheets(color);}catch(err){alert('Color guardado localmente, pero no se pudo enviar a Sheets: '+err.message);} renderConfiguration();
  };
}
  ...
  content.innerHTML=`<section class="module-page products-module">
    <div class="page-actions compact-head">
      ...
      <div class="action-row">
        ...
        <button class="btn btn-secondary compact-btn" id="connectionBtn">⚙ Conexión</button><button class="btn btn-secondary compact-btn" id="syncSheetsBtn">↻ Sincronizar</button><button class="btn btn-primary" id="newProductBtn">+ Nuevo producto</button>
      </div>
    </div>
...
function bindProductShell(){
  ...
  document.getElementById('syncSheetsBtn').onclick=syncFromSheets; document.getElementById('connectionBtn').onclick=()=>navigate('configuracion');
...
async function openProduct(code=null){
  ...
  const brands=[...(db.brands||[]),...db.products.map(x=>x.marca)];
  const categories=[...(db.categories||[]),...db.products.map(x=>x.categoria)];
  ...
  const imageUrl=document.getElementById('imageUrl'); updateImagePreview(imageUrl.value);
...
function updateImagePreview(url){
  ...
  preview.innerHTML=url?`<img src="${escapeHtml(url)}" alt="Vista previa" onerror="this.parentElement.innerHTML='<span>No se pudo cargar</span>'">`:'<span>Sin imagen</span>';
}
async function handleImageUpload(){
  const btn=document.getElementById('uploadImageBtn'),file=document.getElementById('imageFile').files[0],status=document.getElementById('uploadStatus');
  try{btn.disabled=true;btn.textContent='Subiendo…';status.textContent='Enviando imagen a Google Drive…';const url=await uploadImageFile(file,document.getElementById('productForm').elements.codigo.value);document.getElementById('imageUrl').value=url;updateImagePreview(url);status.textContent='Imagen subida y enlace listo para guardar.';}catch(err){status.textContent=err.message;alert(err.message);}finally{btn.disabled=false;btn.textContent='Subir a Drive';}
}
...
function renderVariantList(){
  ...
  el.innerHTML=draftVariants.length?draftVariants.map((v,i)=>{
    const selected=db.colors.find(c=>normalize(c.nombre)===normalize(v.opcion1));
    const colorOptions=db.colors.map(c=>`<option value="${escapeHtml(c.nombre)}" ${normalize(v.opcion1)===normalize(c.nombre)?'selected':''}>${escapeHtml(c.nombre)}</option>`).join('');
    return `<div class="edit-row variant-row" data-index="${i}">
      <div class="variant-color-control"><span class="color-swatch" style="--swatch:${escapeHtml(selected?.hex||'#d7dee8')}"></span><select data-field="opcion1" aria-label="Color"><option value="">Color / modelo</option>${colorOptions}</select></div>
      <input data-field="opcion2" value="${escapeHtml(v.opcion2)}" placeholder="Talla / Capacidad">
      <input data-field="imagen" value="${escapeHtml(v.imagen||'')}" placeholder="URL imagen">
      <input data-field="stock" type="number" min="0" value="${Number(v.stock||0)}" placeholder="Stock">
      <label class="switch-label"><input data-field="activo" type="checkbox" ${v.activo!==false?'checked':''}> Activo</label>
      <button type="button" class="table-action duplicate-variant">⧉</button><button type="button" class="table-action danger remove-variant">×</button>
    </div>`;
  }).join(''):'<div class="mini-empty">Este producto no tiene variantes. Agrega una variante y selecciona un color predeterminado.</div>';
}
...
async function saveProduct(){
  syncDraftFromDom(); const form=document.getElementById('productForm'); const fd=new FormData(form); const data=Object.fromEntries(fd.entries()); if(data.marca==='__NEW__')data.marca=String(data.marcaNueva||'').trim(); if(data.categoria==='__NEW__')data.categoria=String(data.categoriaNueva||'').trim(); delete data.marcaNueva;delete data.categoriaNueva;data.precio=Number(data.precio||0);data.descuento=Number(data.descuento||0);data.variantes=structuredClone(draftVariants);data.mayoreo=structuredClone(draftWholesale).sort((a,b)=>a.desde-b.desde);
  ...
  saveDb(); try{await pushProductToSheets(data);}catch(err){alert('Producto guardado localmente, pero no se pudo enviar a Sheets: '+err.message);} closeDrawer();renderProductsShell();
}
...
content.addEventListener('click',async e=>{
  ...
  if(e.target.closest('#uploadImageBtn'))return handleImageUpload();
  ...
  const del=e.target.closest('.delete-product');if(del&&confirm('¿Eliminar este producto y sus variantes/mayoreo?')){db.products=db.products.filter(x=>x.codigo!==del.dataset.code);saveDb();try{await removeProductFromSheets(del.dataset.code);}catch(err){alert('Se eliminó localmente, pero no en Sheets: '+err.message);}updateProductResults();return;}
...
content.addEventListener('change',e=>{
  ...
  if(e.target.id==='imageUrl')updateImagePreview(e.target.value);
  if(e.target.id==='imageFile'&&e.target.files[0]){const local=URL.createObjectURL(e.target.files[0]);updateImagePreview(local);document.getElementById('uploadStatus').textContent=`Lista para subir: ${e.target.files[0].name}`;}
...
navigate('productos');
if(getApiUrl()){ setTimeout(()=>syncFromSheets(),250); }
