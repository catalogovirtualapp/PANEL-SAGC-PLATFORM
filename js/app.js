...
async function apiRequest(action,payload=null,query={}){
  ...
  try{
    ...
    const options=isPost?{method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({action,...payload}),signal:controller.signal}:{signal:controller.signal,cache:'no-store'};
    const qs=new URLSearchParams({action,t:String(Date.now()),...query});
    const res=await fetch(isPost?url:`${url}?${qs.toString()}`,options);
    if(!res.ok) throw new Error(`Apps Script respondió HTTP ${res.status}. Revisa la implementación.`);
    const text=await res.text();
    let data; try{data=JSON.parse(text);}catch{throw new Error('Apps Script no devolvió JSON. Verifica que uses la URL /exec correcta.');}
    if(!data.ok) throw new Error(data.error||'Error de conexión');
    return data;
  }catch(err){
    if(err.name==='AbortError') throw new Error('La solicitud tardó más de 30 segundos. Revisa Apps Script o la conexión.');
    throw err;
...
if(getApiUrl()){
  setTimeout(async()=>{
    ...
    catch(err){console.warn('No se pudieron cargar catálogos:',err.message);}
  },400);
}
